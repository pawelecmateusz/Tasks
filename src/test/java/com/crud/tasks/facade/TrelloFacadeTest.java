...
class TrelloFacadeTest {
    ...
    void shouldFetchEmptyList() {
        // Given
        TrelloListDto trelloList = new TrelloListDto();
        TrelloBoardDto trelloBoard = new TrelloBoardDto();
        TrelloList mappedTrelloList = new TrelloList();
        TrelloBoard mappedTrelloBoard = new TrelloBoard();

        List<TrelloListDto> trelloLists =
                List.of(new TrelloListDto("1", "test_list", false), trelloList);

        List<TrelloBoardDto> trelloBoards =
                List.of(new TrelloBoardDto("1", "test", trelloLists), trelloBoard);

        List<TrelloList> mappedTrelloLists =
                List.of(new TrelloList("1", "test_list", false), mappedTrelloList);

        List<TrelloBoard> mappedTrelloBoards =
                List.of(new TrelloBoard("1", "test", mappedTrelloLists), mappedTrelloBoard);
    ...
    public void shouldCreateCard() {
        ...
        CreatedTrelloCardDto createdTrelloCardDto = new CreatedTrelloCardDto("1", "name", "url", null);
        ...
        assertEquals("1", trelloService.createTrelloCard(trelloCardDto).getId());
        assertEquals("name", trelloService.createTrelloCard(trelloCardDto).getName());
        assertEquals("url", trelloService.createTrelloCard(trelloCardDto).getShortUrl());
